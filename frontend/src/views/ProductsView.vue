...
onMounted(() => {
  useAxios.get("/products/" + route.params.slug).then((response) => {
...
<template>
  <div>
    <!-- Page Header Start -->
    <PageHeader pageName="CHI TIẾT SẢN PHẨM"></PageHeader>
    <!-- Page Header End -->
    <!-- Shop Detail Start -->
        ...
        <div class="col-lg-5 pb-5">
          ...
          >
            <div class="carousel-inner border">
              <template
                v-for="(image, key) in responseData.product.gallery.split(',')"
                :key="key"
              >
        ...
        <div class="col-lg-7 pb-5">
          ...
          <div class="d-flex mb-3">
            <div class="text-primary mr-2">
              ...
              <template
                v-if="
                  responseData.product.review_avg_rating_value !== null &&
                  !Number.isInteger(
                    responseData.product.review_avg_rating_value
                  ) &&
                  responseData.product.review_avg_rating_value.charAt(
                    responseData.product.review_avg_rating_value.length - 4
                  ) !== '0'
                "
              >
        ...
        <div class="col">
          ...
          >
            ...
            <a class="nav-item nav-link" data-toggle="tab" href="#tab-pane-3"
              >Đánh giá ({{ responseData.product.review_count }})</a
            >
          </div>
              ...
              <div class="row">
                <div class="col-md-6">
                  <h4 class="mb-4">
                    ...
                    đánh giá cho {{ responseData.product.name }}
                  </h4>
                  <template
                    v-for="(data, key) in responseData.product.review"
                    :key="key"
                  >
                        ...
                        <div class="text-primary mb-2">
                          ...
                          <template
                            v-if="
                              ...
                              !Number.isInteger(data.rating_value) &&
                              data.rating_value.charAt(
                                data.rating_value.length - 4
                              ) !== '0'
                            "
                          >
                            <small class="fas fa-star-half-alt"></small>
                    ...
                    <div class="d-flex my-3">
                      ...
                      <template v-if="errorData">
                        <template v-if="errorData['rating_value']">
                          <small
                            v-if="errorData['rating_value'][0]"
                            class="error fw-lighter text-danger text-lg mx-3"
                            >{{ errorData["rating_value"][0] }}</small
                          >
                        </template>
                      </template>
                    </div>
                    <div class="form-group">
                      ...
                      <template v-if="errorData">
                        <template v-if="errorData['comment']">
                          <small
                            v-if="errorData['comment'][0]"
                            class="error fw-lighter text-danger text-lg mx-3"
                            >{{ errorData["comment"][0] }}</small
                          >
                        </template>
                      </template>
                    </div>
                    ...
                    <template v-if="data.offer !== null">
                      <template
                        v-if="
                          ...
                          data.offer.status !== 0 &&
                          data.offer.expire_date > new Date().toISOString()
                        "
                      >
                        <template v-if="data.offer.type == 'Percentage'">
                          <h6 class="text-muted mr-4">
                            {{ Number(data.offer.discount) }}%
                            <small>off</small>
                          </h6>
                        ...
                        <template v-else>
                          ...
                          <h6 class="text-muted ml-2">
                            <del>${{ Number(data.sale_price) }}</del>
                          </h6>
                        </template>
                      </template>
                      <template v-else>
