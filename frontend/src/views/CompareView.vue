...
const { refreshCompareItemsNumber, refreshComparePage } = storeToRefs(
  useRefresh()
);
...
onMounted(() => {
  ...
  if (compareList.value) {
    ...
    useAlert()
      .centerDialogAlert("info", "Your compare list is empty")
      .then((result) => {
        router.back();
      });
  }
...
<template>
  <div>
    <!-- Page Header Start -->
    <PageHeader pageName="SO SÁNH SẢN PHẨM"></PageHeader>
    <!-- Page Header End -->
    <!-- Contact Start -->
    <div class="container-fluid pt-5">
      <div v-if="responseData" class="row px-xl-5">
        <template v-for="(data, key) in responseData.products" :key="key">
          <div
            :class="[compareList.length == 3 ? 'col-lg-4' : 'col-lg-6', 'mb-5']"
          >
            <div
              id="product-carousel"
            ...
            <div class="d-flex mb-3">
              <div class="text-primary mr-2">
                ...
                <template
                  v-if="
                    !Number.isInteger(data.review_avg_rating_value) &&
            ...
            <template v-if="data.offer !== null">
              ...
              >
                ...
                <template v-else>
                  <h5 class="font-weight-semi-bold mb-4">
                    <del
                      ><small>${{ Number(data.sale_price) }}</small></del
                    >
              ...
              <template v-else>
                <h5 class="text-muted ml-2">${{ Number(data.sale_price) }}</h5>
              </template>
            </template>
            <template v-else>
              <h5 class="text-muted ml-2">${{ Number(data.sale_price) }}</h5>
