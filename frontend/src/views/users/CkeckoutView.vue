...
watchEffect(() => {
  if (isAuthenticated.value) {
    useAxios
      .get("/users/checkout/" + user.value.id, {
        headers: { Authorization: "Bearer " + useToken().getToken() },
      })
      .then((response) => {
        responseData.value = response.data;

        if (responseData.value.carts.length === 0)
...
watch(responseData, () => {
  discount = responseData.value.carts.reduce((accumulator, currentValue) => {
    if (
      currentValue["discount"] !== null &&
      currentValue["status"] !== 0 &&
      currentValue["expire_date"] > new Date().toISOString()
    ) {
      if (currentValue["type"] == "Percentage") {
        ...
        return (
          accumulator +
          Math.floor(Number(currentValue["discount"] * currentValue["qty"]))
        );
      }
  ...
  subtotal = responseData.value.carts.reduce((accumulator, currentValue) => {
    return (
      accumulator + Number(currentValue["sale_price"] * currentValue["qty"])
    );
  }, 0);
});
  ...
  useAxios
    .post(
      "/users/checkout/" + user.value.id,
      {
        ...
        coupon: responseCoupon.value
          ? responseCoupon.value.coupon
            ? responseCoupon.value.coupon.discount
            : null
          : null,
        coupon_id: responseCoupon.value
          ? responseCoupon.value.coupon
            ? responseCoupon.value.coupon.id
            : null
          : null,
      },
      {
...
function getCoupon(code) {
  if (code) {
    useAxios
      .get("/users/cart/coupon/" + code, {
        headers: { Authorization: "Bearer " + useToken().getToken() },
      })
      .then((response) => {
        responseCoupon.value = response.data;
        if (responseCoupon.value.coupon == null)
          useAlert().centerDialogAlert("info", "Mã không hợp lệ");
      });
  }
}
</script>
<template>
  <div>
    ...
    <PageHeader pageName="THANH TOÁN"></PageHeader>
      ...
      <div class="row px-xl-5">
        ...
        <div v-if="responseData" class="col-lg-4">
              ...
              >
                <template v-if="responseCoupon.coupon !== null">
                  ...
                  <h6 class="font-weight-medium">
                    ${{ responseCoupon.coupon.discount }}
                  </h6>
                </template>
              </div>
              <div
            ...
            <div class="card-footer border-secondary bg-transparent">
              ...
              >
                ...
                <template v-if="responseCoupon">
                  <h5
                    v-if="responseCoupon.coupon !== null"
                    class="font-weight-bold"
                  >
                    ${{
                      (total = (
                        Number(subtotal) -
                        (Number(responseCoupon.coupon.discount) +
                          Number(discount) +
                  ...
                  <h5 v-else class="font-weight-bold">
                    ${{
                      (total = (
                        Number(subtotal) -
                        (Number(discount) + Number(shippingMethodCost))
                      ).toFixed(2))
                    }}
                  </h5>
                </template>
          ...
          <div class="card border-secondary mb-5">
            ...
            <div class="card-body">
              <template
                v-for="(data, key) in responseData.shippingMethods"
                :key="key"
              >
                <div class="form-group">
                  <div
                    class="custom-control custom-radio d-flex justify-content-between"
                  >
                    <input
                      type="radio"
                      ...
                      :id="key"
                      :value="data.cost"
                      v-model="shippingMethodCost"
                      @click="shippingMethod = data.id"
                    />
              ...
              <template v-if="errorData">
                <template v-if="errorData['shipping']">
                  <small
                    v-if="errorData['shipping'][0]"
                    class="error fw-lighter text-danger text-lg mx-3"
                    >{{ errorData["shipping"][0] }}</small
                  >
                </template>
              </template>
            </div>
          ...
          <div class="card border-secondary mb-5">
            ...
            <div class="card-body">
              <template
                v-for="(data, key) in responseData.paymentMethods"
                :key="key"
              >
                <div class="form-group">
                  <div class="custom-control custom-radio">
                    ...
                    />
                    <label class="custom-control-label" :for="data.type">{{
                      data.type
                    }}</label>
                  </div>
                </div>
              ...
              <template v-if="errorData">
                <template v-if="errorData['payment']">
                  <small
                    v-if="errorData['payment'][0]"
                    class="error fw-lighter text-danger text-lg mx-3"
                    >{{ errorData["payment"][0] }}</small
                  >
                </template>
              </template>
            </div>
