    ...
    useCart()
      ...
      .then((response) => {
        if (response.carts.length === 0) {
          useAlert()
            .centerDialogAlert("info", "Your cart list is empty")
            .then((result) => {
              router.back();
            });
        }
                ...
                <tr>
                  ...
                  <td
                    v-if="
                      data.discount !== null &&
                      data.status !== 0 &&
                      data.expire_date > new Date().toISOString()
                    "
                    class="align-middle"
                  >
                    ...
                    <template v-else> ${{ Number(data.discount) }} </template>
                  </td>
                  <td v-else>0</td>
                  ...
                  <td class="align-middle">
                    ...
                    >
                      <div class="input-group-btn">
                        <button
                          @click="
                            useCart().productQuantityDecrement(
                              data.id,
                              data.p_id,
                              data.qty
                            )
                      ...
                      <div class="input-group-btn">
                        <button
                          @click="
                            useCart().productQuantityIncrement(
                              data.id,
                              data.p_id,
                              data.qty
                            )
                  ...
                  <td class="align-middle">
                    <button
                      @click="useCart().deleteCartItems(data.item_id)"
