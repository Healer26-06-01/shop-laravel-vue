...
<template>
  <div>
    <!-- Page Header Start -->
    <PageHeader pageName="ĐĂNG NHẬP"></PageHeader>
    <!-- Page Header End -->
    <!-- Contact Start -->
    <div class="container">
      <div class="row">
              ...
              >
                <div class="form-group py-2">
                  ...
                  <template v-if="auth.errorData">
                    <template v-if="auth.errorData['email']">
                      <small
                        v-if="auth.errorData['email'][0]"
                        class="error fw-lighter text-danger text-lg mx-3"
                        >{{ auth.errorData["email"][0] }}</small
                      >
                    </template>
                  </template>
                </div>
                <div class="form-group py-1 pb-2">
                  ...
                  <template v-if="auth.errorData">
                    <template v-if="auth.errorData['password']">
                      <small
                        v-if="auth.errorData['password'][0]"
                        class="error fw-lighter text-danger text-lg mx-3"
                        >{{ auth.errorData["password"][0] }}</small
                      >
                    </template>
                  </template>
                </div>
                <div class="form-inline d-flex justify-content-between">
                  ...
                  <RouterLink
                    ...
                    ><a id="forgot" class="font-weight-bold">Quên mật khẩu?</a>
                  </RouterLink>
                </div>
